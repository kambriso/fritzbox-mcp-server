...
Both GitLab CI and GitHub Actions use the golang:1.25 Docker image to ensure consistent toolchain across all builds. Local builds using distribution-provided Go packages (Fedora, Ubuntu, etc.) may produce different binaries due to distro-specific patches.
