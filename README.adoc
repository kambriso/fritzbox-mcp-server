...
Configuration is loaded in the following order (each level can supplement or
override the previous):

1. **Global config**: `~/.config/fritzbox-mcp-server/.env` (created by `--setup`)
2. **Local `.env`**: `.env` in the current working directory (recommended for development)
3. **Environment variables**: OS-level variables (highest priority)

This means a local `.env` works for development, while `--setup` stores credentials
globally for production use with MCP clients.
