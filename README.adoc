...
GitLab releases include SLSA Level 2 provenance via `artifacts-metadata.json` in the job artifacts.

Verification:

[source,bash]
----
# Download artifacts (includes SLSA metadata)
curl -L "https://gitlab.com/jhinrichsen/fritzbox-mcp-server/-/jobs/<JOB_ID>/artifacts/download" -o artifacts.zip

# Extract and examine provenance
...
All binaries are signed using https://docs.sigstore.dev/cosign/[Cosign] with keyless signing (OIDC). Signatures and certificates are included in the release artifacts.
