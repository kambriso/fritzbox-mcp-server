...
* No hardcoded TR-064 service structs
* Discovers capabilities dynamically from your FRITZ!Box's XML descriptors
...
FRITZ_PORT=49000              # TR-064 port (default: 49000)
...
Fetch TR-064 XML descriptors without authentication:
...
├── xml/                    # Cached TR-064 XML descriptors
├── dist/                   # Built binaries (created by make dist)
├── config.go               # Configuration loading
├── tr064.go               # TR-064 client implementation
...
This server is designed to be runtime-driven and model-agnostic. It does not contain any hardcoded TR-064 service structures.

==== Design Principles

* No static, model-specific Go structs for TR-064 services
* No dependency on any cloud for auth, logic, or docs
* Discovery of TR-064 services/actions from runtime XML
* Typed MCP tools generated at runtime (from XML + PDFs)
...
===== TR-064 Core Client (internal/tr064)
...
All TR-064 argument values represented as map[string]string.

Authentication:

* Implements AVM TR-064 auth (SOAP/ClientAuth variant)
* First request: without auth → expect challenge response from FRITZ!Box
* Parse nonce/realm/etc. from response
* Compute MD5-based token (per AVM TR-064 First Steps doc)
...
No TR-064 field names are hardcoded outside this package.

===== TR-064 Spec Registry (internal/spec)
...
* Each property name is the TR-064 arg name (e.g. NewMACAddress)
...
call_action::
Execute any TR-064 action on the FRITZ!Box. Use list_services and describe_action to discover available actions and their parameters.
...
=== TR-064 Services Reference

Generated from:

* https://fritz.support/schnittstellen/TR-064_overview.pdf[TR-064 Support – Overview (PDF)]
* https://fritz.support/resources/TR-064_Device_Info.pdf[TR-064 Support – DeviceInfo (PDF)]
* https://fritz.support/resources/TR-064_First_Steps.pdf[TR-064 – First Steps (PDF)]
* https://fritz.com/pages/schnittstellen[FRITZ! Interfaces overview page]

==== Standard TR-064 services
...
NOTE: Actual availability per device (FRITZ!Box vs Repeater) is defined in the TR-064 Overview table; your runtime client should always discover services from `/tr64desc.xml`.

==== Example Service: deviceinfo

Service name:: `deviceinfo`
Service ID (SCPD):: `urn:DeviceInfo-com:serviceId:DeviceInfo1`
Spec:: https://fritz.support/resources/TR-064_Device_Info.pdf[TR-064 Support – DeviceInfo (PDF)]
...
* `NewSpecVersion` → TR-064 spec version implemented
